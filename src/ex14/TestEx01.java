...
/*
	...
	풀고자 하는 문제를 다른 문제로 변환한다 -> 변환된 문제의 답을 구한다. -> 구한 답을 원래 문제의 답으로 삼는다.
...
public class TestEx01 {
	...
	public static int input() {
		...
		int x = sc.nextInt();
		System.out.println("입력한 수 : " + x);
		return x;
	}
	
	public static int multipleThree(int a) {
		// 연산이 어디까지 진행되는지 확인하기 위한 변수 count 
		int count = 0;
		int num = a;
		// while문을 통해 3,6,9중 하나가 될때까지 반복
		// 어느 자리 수가 들어올지 모르므로, String 타입으로 바꾼뒤 문자열을 하나씩 쪼개서 하기
		
		while(num != 3 && num != 6 && num != 9) {
			// num이 3, 6, 9이면 while문 종료
			if(num<10) {
				// num이 한자리 수인데, 조건문에 안걸리는 경우이므로 3의 배수가 아님
				answer = false;
				break;
			}else {
				int sum = 0;
